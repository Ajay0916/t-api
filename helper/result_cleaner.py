...
def clean_results(resp, sort=True, dedup=True):
    ...
    for item in data:
        if isinstance(item, dict):
            ...
            # Site RSS feeds occasionally leak debug junk into titles
            # (newlines, <br> tags, php notices) - normalize before the
            # title is used for links/filenames/dedup.
            if item.get("name"):
                name = re.sub(r"<br\s*/?>", " ", str(item["name"]), flags=re.I)
